				...
				int prev_fd, int *pipefd)
{
	...
	if (pid == 0)
	{
		...
		exec_pipeline_child(current, cmds, envp, prev_fd, pipefd);
	}
	return (pid);
}

static void	execute_commands_loop(t_cmd *cmds, char **envp, pid_t *pids)
{
	t_cmd	*current;
	int		prev_fd;
	int		cmd_index;
...
void	exec_pipeline(t_cmd *cmds, char **envp)
{
	...
	if (!pids)
		return ;
	execute_commands_loop(cmds, envp, pids);
