			...
			int prev_fd, int *pipefd)
{
	...
	if (is_builtin(current))
		handle_builtin_in_pipeline(current, cleaned_args, cmds);
	else
		handle_external_in_pipeline(cmds, envp, cleaned_args);
}
			...
			int *prev_fd)
{
	...
	if (create_pipe_if_needed(current, pipefd) == -1)
		return (-1);
	pid = fork_and_handle_child(current, cmds, envp, *prev_fd, pipefd);
	if (pid == -1)
		return (-1);
